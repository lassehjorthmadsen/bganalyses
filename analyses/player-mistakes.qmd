...
params:
  player_name: "Llabba"
  analysis_folder: "\\analyzed\\4-ply"
  max_examples: 20
  seed: 1970
  benchmark_name: "lasse"

title: "Mistakes were made by `r params$player_name`"
...
# Notes:
# quarto preview analyses/player-mistakes.qmd --cache-refresh
...
```{r parse}
#| cache: true
file_path <- paste0("..\\..\\bglab\\data-raw\\", params$player_name, params$analysis_folder)
...
df_bench <- moves %>%
  ...
  filter(!is.na(cost), cost <= -min(cost_breaks), turn %in% c(params$player_name, params$benchmark_name)) %>% 
  mutate(turn = fct(turn, levels = c(params$benchmark_name, params$player_name)),
...
df <- df_bench |> filter(turn == params$player_name)
...
The report was created like this: First, I use a Python script for GNU Backgammon to analyze a collection of matches at 4-ply. Then, my R-package [`bglab`](https://lassehjorthmadsen.github.io/bglab/index.html) contains tools to turn the GNU BG analysis files into a data frame that can be analyzed using descriptive statistics. Finally, this Quarto notebook (part of my [Backgammon Playground](https://lassehjorthmadsen.github.io/bganalyses/) with R-code analyzes that data. 

The data are build from a set of `r length(files)` matches played by the nick `r params$player_name`. The matches contains of a total of `r nrow(moves)` decisions, from which we look at the subset of `r nrow(df)` decisions by `r params$player_name`, that GNU BG thought was in error.
...
- The number of cube errors made 
- The sum all cube errors
- The average size of cube errors
...
plot_data <- df_bench %>% 
  ...
  mutate(metric = fct(metric, levels = metric_labels),
         width = case_when(turn == params$player_name ~ 0.8, TRUE ~ 0.4),
         alpha = case_when(turn == params$player_name ~ 0.5, TRUE ~ 0.5))
