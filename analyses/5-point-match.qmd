...
#| fig-height: 5
score_order <- tps |> 
  filter(cube == 1, pos_type == "Race", x <= 5, y <= 5) |> 
  arrange(-tp_dead) |> 
  pull(score_label)
...
plot_data <- tps |> 
  filter(cube == 1, pos_type != "Last roll", x <= 5, y <= 5) |> 
...
At the score recubes are even stronger: Black will need `r tp(4, 5, 2, met) |> round(3)` to take the recube so must pass in case of a miss. That means Whites chances are effectively $\frac{10}{36}\approx 0.278$ -- but still falling short of the `r tp(5, 4, 1, met, T) |> round(3)` last-roll take point. \`
...
-   In a last roll position White still has a high take point at `r tp(5, 3, 1, met) |> round(3)`, because two points are very valuable for Black -- getting him to the Crawford game
...
Bottom line: White has a marginal take/pass decision. 22% is about what he needs, the occasional freak gammon *win* (probability about 3%) accounts for the difference from the mentioned `r tp(5, 2, 1, met) |> round(2) |> label_percent()()` racing game dead cube take point.
...
The slightest smell of blitz, makes for at best a marginal take at 4-away, 2-away.
...
-   With the perfect automatic recube available the trailer can of course take with `r met3[4,1] |> label_percent()()` -- the winning chance we get when passing for 4-away, 1-away, Crawford.
...
Black leads in every area, including the important race, so solid double/take for money. But 3-away, 3-away pushes this into double/pass territory.
...
ggboard("XGID=-DD-------------------acd-:0:0:-1:D:3:2:0:5:10") +
  labs(title = "Example position 1: White wins 25.5%",
       subtitle = "An imperfect four-roll position; take for money, pass at the score")
"
Money game, Cubeful Equities:
...
In this 4-roll position 11 doesn't necessarily save a roll for White, so White is a bit worse off compared to a pure 4-roll position. Enough to turn a money take into a pass at the score.

```{r}
#| column: page-right
#| layout-ncol: 2

ggboard("XGID=-b----E-C---cD---bbd--AbAA:0:0:-1:D:3:2:0:5:10") +
  labs(title = "Example position 2",
       subtitle = "Take for money, big pass at the score")
...
When Black has a match winning gammon threat, this small money take becomes a gigantic pass at 2-away, 3-away
...
ggboard("XGID=-B-------------------a-a--:0:0:-1:D:2:3:0:5:10") +
  labs(title = "Example position 1. White wins 36.1%",
       subtitle = "Beware the last-roll position at this score")
...
Even with 36.1% winning chances White can't quite take at the score with no automatic recube available; he needs 36.8%.

```{r}
#| column: page-right
#| layout-ncol: 2

ggboard("XGID=-DD-------------------acd-:0:0:-1:00:3:2:0:5:10") +
  labs(title = "Example position 2: White wins 25.5%",
       subtitle = "An imperfect four-roll position; take for money, take at the score")
"
Money game, Cubeful Equities:
       No double:     +0,764 (-0,141)
       Double/Take:   +0,906
...
The same near-four-roll position as shown to be a pass at 2-away, 3-away. A smallish take at this score.

### The funny two-point match

The two-point match is funny: It's a well-known fact that an optimal strategy is to double at first opportunity, at which point the opponent will always have a clear take, with the take point being `r round(met[2,1] * 100, 0)`%.
...
a)  The opponent doubling too late and loosing their market
b)  The opponent passing a less obvious take

On the flip side, delaying doubling may cost you if:

a)  You accidentally lose your market by overlooking something
b)  You end up passing what is really a take.
...
$Take\ point = \frac{loss}{loss+gain} = \frac{1.2}{1.2 + 48.8} = `r round(1.2/50 * 100, 1)`$%

At 1-away, 5-away, the corresponding numbers would look like this:

$Take\ point = \frac{loss}{loss+gain} = \frac{1.3}{1.3 + 32.3} = `r round(1.3/33.6 * 100, 1)`$%
...
```{r}
compare_tps(x1 = 5, y1 = 5, cube1 = 2, tps = tps, tpsm = tpsm)
```
...
ggboard("XGID=-B-bBCCBA---b----c-ccb-AA-:1:-1:-1:00:0:0:0:0:10") +
  labs(title = "Example position 1",
       subtitle = "Moderate take for money, marginal take at the score")
...
5-away, 5-away, Cubeful Equities:
       ...
       Redouble/Take:   +0,996
       Redouble/Pass:   +1,000 (+0,004)
" |> remove_stuff() |> cat()
```

Case in point: A position with strong boards and some gammon potential for both sides, but not enough to reverse the small tendency to drop at the score: White has enough for a decent money-take for, but at best a very marginal take at the score.
...
ggboard("XGID=b---C-CBB---dC---c-cc-B---:1:1:1:00:0:0:0:5:10") +
  labs(title = "Example position 2",
       subtitle = "Small take for money, clear take at the score")
...
Ignoring that this position is too young to be a likely *re*double, Black is more inclined to take at the score, than for money.

### 4-away, 5-away

```{r}
compare_tps(x1 = 4, y1 = 5, cube1 = 2, tps = tps, tpsm = tpsm)
```
...
```{r}
compare_gvs(4, 5, c(2,4), met)
```

```{r}
#| column: page-right
#| layout-ncol: 2
ggboard("XGID=-B-bBCCBA---b----c-ccb-AA-:1:-1:-1:00:1:0:0:5:10") +
  labs(title = "Example position 1",
       subtitle = "Moderate take for money, huge drop at the score")
...
Example position 1, which was a marginal take at 5-away, 5-away, is now a huge drop.

### 3-away, 5-away

```{r}
compare_tps(x1 = 3, y1 = 5, cube1 = 2, tps = tps, tpsm = tpsm)
```
...
#| column: page-right
#| layout-ncol: 2
ggboard("XGID=-AbDCBD----aca----bbbb--A-:1:-1:-1:00:2:0:0:5:10") +
  labs(title = "Example position 1",
       subtitle = "Clear double/take for money, huge drop at the score")
...
A pretty straight double/take for money, becomes a gigantic drop at the score.

### 5-away, 4-away

```{r}
compare_tps(x1 = 5, y1 = 4, cube1 = 2, tps = tps, tpsm = tpsm)
```
...
#| column: page-right
#| layout-ncol: 2
ggboard("XGID=-DCC-------------------ee-:1:-1:-1:00:0:1:0:5:10") +
  labs(title = "Example position 1: White wins 23.7%",
       subtitle = "Moderate take for money, clear drop at the score")
...
This is a 5-roll position with drawbacks: White can't use 22 to save a roll and 11 is not fully effective. Those are quite expensive drawbacks, rather than 28% in in a pure 5-roll, white has only 24% here. Enought for a good money take, not enough at 5-away, 4-away, even taking into consideration the automatic recube.

```{r}
#| column: page-right
#| layout-ncol: 2
ggboard("XGID=---bBBDBBA--A-aa--abbbbb-A:1:-1:-1:00:0:1:0:5:10") +
  labs(title = "Example position 2",
       subtitle = "Clear pass for money, no double at the score")
...
This looks scary for money, and it is indeed a pass. But of course the many gammons don't matter after double/take/re-double, so White has a super easy take and Black is better off not doubling.

### 5-away, 3-away

```{r}
compare_tps(x1 = 5, y1 = 3, cube1 = 2, tps = tps, tpsm = tpsm)
```
...
#| column: page-right
#| layout-ncol: 2
ggboard("XGID=---BBBCB----aB----Bccbbbb-:1:-1:-1:00:0:2:0:5:10") +
  labs(title = "Example position 1. White wins 18.2%",
       subtitle = "Big pass for money, clear take at the score")
...
5-away, 3-away, Cubeful Equities:
       ...
       Redouble/Take:   +0,870
       Redouble/Pass:   +1,000 (+0,130)
" |> remove_stuff() |> cat()
```

White can find the 15.8% he needs to take here.

## The rare 8-cube

```{r}
compare_tps(x1 = 5, y1 = 5, cube1 = 4, tps = tps, tpsm = tpsm)
```
...
#| column: page-right
#| layout-ncol: 2
ggboard("XGID=---BBBBB-----C-------bbbB-:2:-1:-1:00:0:0:0:5:10") +
  labs(title = "Example position 1: White wins 16.5% at the score",
       subtitle = "Too good to double, take")
...
Black is crazy too-good for money, but both too-good and not good enough at 5-away, 5-away holding a 4-cube.
