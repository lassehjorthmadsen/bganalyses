...
live <- tp_table$`Dead-cube take point` # Initalize live tp values

# Calulate live tp values starting with second-to-last double
...
These are all the same numbers that GNU BG and XG reports, in the "Market window" and "Cube Information" dialog boxes. 



### Example: 16-away, 17-away

For fun, let's study the same example for a longer match; now we're being initially doubled when trailing 17-away, 16-away, (-16, -17). Since the recube vig propagates all the way through redouble after redouble, we have to start by considering the last possible double. Counting at fingers, we can discover that the last possible double (after multiple turn-arounds and cube actions), would be us offering a 16-cube which will be dead hereafter.
