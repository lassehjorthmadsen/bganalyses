...
This is just another way to express the familiar take point formula, most often written like this: $TP= \frac{Risk}{Gain + Risk}$
...
Applying this to backgammon, let's assume that we accept a cube with winning chances of $0.2$ and that we can double our opponent efficiently out if we reach a winning chance of $0.8$. What are our actual winning chances then, assuming a perfectly continuous game? We need to improve from $0.2$ to $0.8$ before we deteriorate from $0.2$ to $0.0$ so $a=0.2, b=0.6$, and:

$$P(E) = \frac{0.2}{0.2+0.6} = \frac{1}{4} = 0.25$$

In other words: The optimistic live-cube model would allow us to take a gammonless game with a cubeless winning probability of just $0.2$ since cube access could turn that into the necessary $0.25$ winning probability. (Keeler and Spencer has a more formal proof of the $0.2$ live-cube take point.)
...
tp_table <- tibble(
  ...
  `Opponent's take point` = c(round(tp_money(probs_fliped, 0), 4),
...
Putting aside for a moment the take points, let's look at the *equities*, i.e. the average or *expected* value of a position. For White in the above position, using the Janowski formulas, we get:

```{r}
p <- sum(probs[1:3])
x <- 0.706

tibble(`Cube location` = c("Owned by White", "Centered", "Owned by Opponent"), 
       `Equity` = c(round(eq_money(probs = probs, C = 1, p = p, x = x), 3),
...
tibble(`Cube location` = c("Owned by White", "Centered", "Owned by Opponent"), 
       `Equity` = c(round(eq_money(probs = probs, C = 1, p = p, x = x), 4),
...
-   White's equity owning the cube: `r round(eq_money(probs = probs, C = 1, p = p, x = x), 4)`
-   White's equity with a centered cube: `r round(eq_money(probs = probs, C = 0, p = p, x = x), 4)`
-   White's equity after double/take: `r round(2 * eq_money(probs = probs, C = -1, p = p, x = x), 4)`
...
Our take point assuming a dead cube is $`r round(tp(5,3,1,met), 3)`$ (we skip the actual calculation; it is just the standard one, that I mentioned earlier, $TP= \frac{Risk}{Gain + Risk}$)
...
In a funny way, *all* the take points affects our take point for the initial double, since the calculations are recursive, with each take point affecting the next.
...
Okay, so far it seems possible to (more or less) reproduce how XG and GNU BG transforms cubeless distributions into cubeful match equities. Let's try a match play example with gammons.

Here's a standard opening blitz after 63, 55, dance:
...
Do we have a double? Can our opponent take?

Applying the Janowski formulas, we get take points as follows:


```{r}

tp_info_pl <- tp_info(4, 7, probs, 1, met)
tp_info_op <- tp_info(7, 4, probs_fliped, 1, met)

tp_table <- tibble(
  Cube = c("Dead-cube", "Live-cube", "Real-cube"),
  `Opponent's take point` = c(round(tp_info_op["tp_dead"], 4),
                           round(tp_info_op["tp_live"], 4),
                           round(tp_info_op["tp_real"], 4)),
  `Player's take point` = c(round(tp_info_pl["tp_dead"], 4),
                           round(tp_info_pl["tp_live"], 4),
                           round(tp_info_pl["tp_real"], 4))
)
