...
- White's equity owning the cube: `r round(eq_janowski(probs = probs, C = 1, p = p, x = 0.706), 3)`
- White's equity with a centered cube: `r round(eq_janowski(probs = probs, C = 0, p = p, x = 0.706), 3)`
- White's equity after double/take: `r round(2 * eq_janowski(probs = probs, C = -1, p = p, x = 0.706), 3)`
...
```{r}
probs <- outcome_probs(c(49.69, 0, 0, 50.31, 0, 0))/100
probs %>% 
  probs_table() %>% 
  kable("html") %>%
  column_spec(2:5, width = "3.1cm") 
