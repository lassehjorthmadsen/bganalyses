...
Since the recommended take point, aka the "real-cube take point" is just a (weighted) average of the dead-cube and the live-cube take points, both bots would presumable also be more conservative on that -- although none of them report it. 

This puzzled me for a long while, since I thought the extention of Janowski's formulars to matchplay was pretty straighforward, and that the way I did it was in the spirit of the money game variant. 

As it turns out, the developers of XG and GNU BG chose to do it a little differently. When calculating the first step, the dead-cube take point, they basically igore the gammons that the *taker* might win. Apparently they did so on the theory the taker would double out, *before* winning any of those gammon.
...
So looking at the earlier formular:

$$TP_{live} = TP_{dead}\times(1-TP_{opponent})$$

Both XG and GNU BG calculates $TP_{dead}$ with zero gammons wins for the taker (but not for the doubler). That clearly yields a higher $TP_{dead}$ and consequently also a higher $TP_{live}$. Hence the discrepancy.

So which implementation is better?

Hard to say. We don't really have a good way of emperically establishing the true take point in a position with those exact gammon and backgammon frequencies. In the example position, a rollout has Black winning (after double and take) with probability 0.341, and a normalized equity of -0.962, so 0.341 is enough for a smallish take. 

From that perspective, my estimated real-cube take point of 0.3251 could be on the low side.

On the other hand XG 3-ply thinks Black wins 0.341 (just like in rollout) for an equity of -0.974, slightly worse off than the rollout showed. GNU BG's equivalent 2-ply thinks Black wins 0.329 for a pass of -1.079, considerbly worse than the rollout, but partly due to being more pessimistic about the position itself.

Any way, this is really splitting hairs, and it's only one sample position, so basically impossible to say which model is better for estimating the take point you *really* need, at a given score, with given gammon frequencies.
...
Much as I try, I can't complete figure out exactly, what would be the right way to use Janowski or a Janowski-like model in match play when gammons count.
