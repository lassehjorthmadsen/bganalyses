...
In other words: The optimistic live-cube model would allow us to take a gammonless game with a winning probability of just $0.2$ since cube access could turn that into the necessary $0.25$. (Keeler and Spencer has a more formal proof of the $0.2$ live-cube take point.)
...
From the table, we can note that the "real-cube take point" is $0.217$, about the same as for money, which is not surprising in a straight race at the begining of a long match. We could also note our opponent's take point, should we ever get to offer him a 16-cube, is somewhat low for a dead cube: $0.210$. That's because the 16 points he can win is exactly what he needs to win the match, so they are slightly more valuable the 16 points we can win.

In a funny way, this take points affects *our* take point for the initial double, since the calculations are recursive, with each take point affecting the next.

To see this more clearly, we can write out the full calculations. For simplicity, we use $TP_1, TP_2, TP_4, and\ TP_8$ to indicate the dead-cube take points when the cube is at 1, 2, 4, 8 respectively. (The column "Dead-cube take point" from the table.)

Then:

$$TP_{1_{live}} = TP_1 \times (1 - TP_2 \times (1 - TP_4 \times (1- TP_8)))$$
Which can be written as:

$$TP_{1_{live}} = TP_1 - TP_1 \times TP_2 + TP_1 \times TP_2 \times TP_4 - TP_1 \times TP_2 \times TP_4 \times TP_8$$
