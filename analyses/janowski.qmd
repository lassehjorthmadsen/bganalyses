...
devtools::load_all(path = here::here("..", "bglab"))
met <- get_met()
```

## Update

*This note is updated to reflect a more complete understanding of how bots estimate live cube take points. Jeremy Bagai nudged me to update it; Minh Nguyen provided some important missing pieces; Leonard Brown pointed out an error. Thanks guys.*

## Why this?

This note is for better understanding exactly what eXtreme Gammon and GNU Backgammon do to estimate cubeless and cubeful equities at money and match play. In particular, we apply the Janowski Formula(s) and try to figure out what it means when the eXtreme Gammon Documentation says that "In eXtreme Gammon the Janowski formula has been extended to apply also in match play".

## How XG and GNU BG works

The basic mechanism is this:

-   A neural net evaluates a position and comes up with six probabilities for either winning or losing a regular game, a gammon and a backgammon. The estimated probabilities are for cubeless money game: What will happen if the game is played to conclusion with no doubling cube at money game conditions.
...
As mentioned, a game of backgammon played to conclusion can end in six possible ways. A certain position in Janowski's article, has the following probabilities of each outcome:
...
The *real* take point for most positions is somewhere between these two extremes: Lower than the pessimistic dead-cube take point, but higher than the overly optimistic live-cube take point.

### The continuous model

To calculate the live-cube take point, we need to briefly visit the so-called continuous model, proposed by Keeler and Spencer, as far back as 1975, in an article called [*Optimal Doubling in Backgammon*](https://bkgm.com/articles/KeelerSpencer/OptimalDoublingInBackgammon/index.html).
...
This applies to any game where winning chances move continuously as time passes. Keeler and Spencer have a nice example from basketball, paraphrased like this:
...
In other words: The optimistic live-cube model would allow us to take a gammonless game with a cubeless winning probability of just $0.2$ since cube access would turn that into the necessary $0.25$ winning probability. (Keeler and Spencer have a more formal proof of the $0.2$ live-cube take point.)
...
Incorporating this into the Janowski formula for take points, we can get an estimated, "real" take point, that takes recubes *and* cube efficiency into account:
...
Expanding the above ideas, the Janowski-formulas can be rearranged to express the *equity*, as a function of game winning chances, $p$, cube efficiency, $x$, and cube location (*player*, *opponent*, *center*):

$$E_{player}= p(W+L+0.5x)-L$$ 
$$E_{opponent}= p(W+L+0.5x)-L-0.5x$$ 
$$E_{center}= \frac{4}{4-x}(p(W+L+0.5x)-L-0.25x)$$
...
Note that the numbers that XG reports are probabilities for winning (*including* gammons and backgammons), gammon (*including* backgammon), and backgammon. They have to be converted to get probabilities that sum to 1, like in the above table, if one prefers that way of looking at it. (My R-package, [`bglab`](https://lassehjorthmadsen.github.io/bglab/index.html) has a function for that.)

If we apply Janowski's take point formulas to the above cubeless probabilities, we get:
...
All these four numbers align perfectly with what you get when you consult XG's `Analyze|Cube Information` menu. GNU Backgammon, in `Analyze|Market Window`, also gives the exact same take points. We can conclude that both programs probably implemented Janowski's take point formula in the same way for money games.
...
tibble(`Cube location` = c("Owned by Player", "Centered", "Owned by Opponent"), 
       `Calculated Equity` = c(round(eq_money(probs = probs, C = 1, p = p, cube_eff = x), 3),
                    round(eq_money(probs = probs, C = 0, p = p, cube_eff = x), 3),
                    round(2 * eq_money(probs = probs, C = -1, p = p, cube_eff = x), 3)),
       `XG reported Equity` = c(0.181, 0.002, -0.344)
       ) %>%
  kable("html") 
```

The calculated equities agree pretty closely with what XG says. 

Note that XG's reported equity listed as "Owned by Opponent" is doubled, so you can directly compare a non-double action to a double/take action. This is why it looks like Opponent (Black), has about double the equity as Player (White), if owning the cube.
...
The [GNU Backgammon Manual](https://www.gnu.org/software/gnubg/manual/gnubg.html), helpfully states that they use a cube-life index, or cube efficiency, of $0.68$ for contact positions. Applying Janowski's formulas for cubeful equity to the above probabilities and, using $0.68$ as cube efficiency, we get:
...
tibble(`Cube location` = c("Owned by Player", "Centered", "Owned by Opponent"), 
       `Calculated Equity` = c(round(eq_money(probs = probs, C = 1, p = p, cube_eff = x), 4),
                    round(eq_money(probs = probs, C = 0, p = p, cube_eff = x), 4),
                    round(2 * eq_money(probs = probs, C = -1, p = p, cube_eff = x), 4)),
...
Going back to the question of which cube-life index XG uses: Perhaps it depends on the position? (GNU BG uses a slightly lower value, $0.60$ rather than $0.68$, in short races.)

Let's do another example to try to find out.
...
```{r}
tibble(`Cube location` = c("Owned by Player", "Centered", "Owned by Opponent"), 
       `Estimated Equity` = c(round(eq_money(probs = probs, C = 1, p = p, cube_eff = x), 4),
                    round(eq_money(probs = probs, C = 0, p = p, cube_eff = x), 4),
                    round(2 * eq_money(probs = probs, C = -1, p = p, cube_eff = x), 4)),
...
The values that XG reports are very close to the calculated values, so it must assume a lower cube value in the second position, which would also be reasonable in a medium to short length race like this.
...
XG's values are again close to the calculated values, although not exactly the same. The reason for the discrepancy (especially with the cube in the middle) is not obvious. (I used 1-ply to make sure the difference is not due to how the numbers get aggregated at higher plies.)
...
Let's see if we can generalize the Janowski formulas to match play. The loss, L, and win, W, part from the money game section above, should be easy enough: We can just plug in match equities from Kazaross XG2 match equity table, to get the average $mwc$ when losing or winning respectively.
...
The intuition behind this is, that when we hold the cube, we are "in the game until the end," that is, until our winning chances are zero, but we don't have to go to 100% to win; when we have winning chances of $0.8$, we can use the cube to effectively win the game.
...
These are all the same numbers that GNU BG and XG reports, in the "Market window" and "Cube Information" dialog boxes. A rollout confirms that in a long race (I tried 125 pips vs 139 pips), trailing (-5,-3), you do indeed have a small take with about 22% winning chances, just like the "real-cube" average indicates.
...
For fun, let's study the same example for a much longer match; now we're being initially doubled when trailing 25-away, 24-away, (-25, -24). Since the recube vig propagates all the way through redouble after redouble, we have to start by considering the last possible double. Counting on fingers, we can discover that the last possible double (after multiple turn-arounds and cube actions), would be when the opponent owns a 16-cube, and we need to decide if we should pass and trail (-25, -8) or accept a 32-cube which will be dead hereafter.
...
In a funny way, *all* the possible future take points affect our take point for the initial double, since the calculations are recursive, with each take point affecting the next.
...
\end{align*}
$$
So it is clear that the big effect on the live-cube take point of course is from the actual dead-cube take point, at the same cube level. Our take point if we eventually get redoubled from 16 to 32, $TP_{16}$, plays very little role, and is only of theoretical interest. Our opponent's take point when we redouble, $TP_2$, on the other hand, does play a role, as the (-5,-3) example showed. 
...
```
We see how we get from the initial dead-cube take point of `r round(tp(25,24,1,met), 3)` to the live-cube take point of `r round(tp_info(25,24,c(0.5,0,0,0.5,0,0),1,met,0.7)["tp_live"], 3)`. Each subsequent take point has less and less of an impact on our initial take point. And let's just remember once again that the real take point is to be found between these two values; when the game still has a way to go, it is usually closer to live-cube take point than to the dead-cube take point.

### Example with gammons

Okay, so far it seems possible to (more or less) reproduce how XG and GNU BG transforms cubeless distributions into cubeful match equities and take points. Let's try a match play example with lots of gammons, with us playing White and leading (-4,-7).
...
So, what's the bottom line? Do we have a double? Can our opponent take? 
...
In the last row we see that Black has a very high take point, `r round(tp_info_initial["tp_dead"], 4)`, for the reason mentioned: The likely gammon will lose the match. Should Black turn the game around, however, White *also* has a high take point, `r round(tp_info_recube["tp_dead"], 4)`, and since the cube would be dead, the live-cube take point is the same as the dead-cube take point. For that reason, Black's live-cube and real-cube take points are much lower: The model suggests Black would be able to take with a winning probability of `r round(tp_info_initial["tp_real"], 4)`. 

GNU BG reports the exact same dead-cube take point, which is not surprising.

The live-cube take point, or "Fully live" as GNU BG calls it, is however a bit different: GNU BG reports 0.3012, rather than the `r round(tp_info_initial["tp_live"], 4)` that I get. XG shows similar figures.

In other words, GNU BG and XG are both *less* optimistic about the theoretical minimal take point, aka the live-cube take point, than my implementation of Janowski. But the discrepancy arises only in matchplay, and when gammons are in play.

Since the recommended take point, aka the "real-cube take point" is just a (weighted) average of the dead-cube and the live-cube take points, both bots would presumably also be more conservative on that -- although none of them report it. 

This puzzled me for a long while, since I thought the extension of Janowski's formulas to match play was pretty straightforward, and that the way I did it was in the spirit of the money game variant. 

As it turns out, the developers of XG and GNU BG chose to do it a little differently. When calculating the first step, the dead-cube take point, they basically ignore the gammons that the *taker* might win. Apparently they did so on the theory that the taker would double out, *before* winning any of those gammons.

(This was kindly explained by Minh Nguyen in the Backgammon Strategy Facebook group [here](https://www.facebook.com/groups/backgammonstrategy/posts/3532552207013426/?comment_id=3631176307151015&notif_id=1718828064640279&notif_t=group_comment). Thank you Minh!)

So looking at the earlier formula:

$$TP_{live} = TP_{dead}\times(1-TP_{opponent})$$

Both XG and GNU BG calculate $TP_{dead}$ with zero gammon wins for the taker (but not for the doubler). That clearly yields a higher $TP_{dead}$ and consequently also a higher $TP_{live}$. Hence the discrepancy.

So which implementation is better?

Hard to say. We don't really have a good way of empirically establishing the true take point in a position with those exact gammon and backgammon frequencies. In the example position, a rollout has Black winning (after double and take) with probability 0.341, and a normalized equity of -0.962, so 0.341 is enough for a smallish take. 

From that perspective, my estimated real-cube take point of 0.3251 could be on the low side.

On the other hand XG 3-ply thinks Black wins 0.341 (just like in rollout) for an equity of -0.974, slightly worse off than the rollout showed. GNU BG's equivalent 2-ply thinks Black wins 0.329 for a pass of -1.079, considerably worse than the rollout, but partly due to being more pessimistic about the position itself.

Anyway, this is really splitting hairs, and it's only one sample position, so basically impossible to say which model is better for estimating the take point you *really* need, at a given score, with given gammon frequencies.
...
Also, the Janowski transformation appears implemented differently from what I thought was the natural way.

Much as I try, I can't completely figure out exactly, what would be the right way to use Janowski or a Janowski-like model in match play when gammons count.

Still, the exercise feels useful.